...
A = np.array([[-0.8, -0.22], [1, 0]])  # from Moore
B = np.array([0.5, 1])  # grr lti [:,np.newaxis]
C = np.array([1, 0.5])
D = np.zeros(1)
niter = 20
...
np.set_printoptions(
    threshold=10, edgeitems=3, linewidth=150, formatter=dict(float=lambda x: "%.2g" % x)
)  # float arrays %.2g
np.random.seed(seed)
...
for rate, ax in zip(rates, axes):
    ...
    for iter in range(niter):
        ...
        deltaU = np.fabs(U - Us[-1]).mean() if len(Us) > 0 else np.NaN

        print(
            "err: %2d: av %-8.2g  max %-8.2g  dU %-8.2g  %s "
